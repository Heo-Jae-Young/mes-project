...
class CCP(models.Model):
    ...
    process_step = models.CharField(
        max_length=200, help_text="어느 공정 단계의 CCP인지"
    )
    critical_limit_min = models.DecimalField(
        max_digits=10, decimal_places=3, null=True, blank=True
    )
    critical_limit_max = models.DecimalField(
        max_digits=10, decimal_places=3, null=True, blank=True
    )
    monitoring_frequency = models.CharField(
        max_length=100, help_text="모니터링 주기 (예: 매 30분)"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="created_ccps"
    )
...
class CCPLog(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        User, on_delete=models.PROTECT, related_name="created_ccp_logs"
    )
    ...
    def __str__(self):
        return (
            f"{self.ccp.name} - {self.measured_value} {self.unit} ({self.measured_at})"
        )

    def save(self, *args, **kwargs):
        """저장 시 자동으로 한계 기준 체크"""
        if self._state.adding:  # 새로 추가되는 객체인지 확인
            if (
                self.ccp.critical_limit_min is not None
                and self.measured_value < self.ccp.critical_limit_min
            ):
                self.is_within_limits = False
                self.status = "out_of_limits"
            elif (
                self.ccp.critical_limit_max is not None
                and self.measured_value > self.ccp.critical_limit_max
            ):
                self.is_within_limits = False
                self.status = "out_of_limits"
            else:
